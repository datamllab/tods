...
# Step 4: algorithm
step_4 = PrimitiveStep(primitive=index.get_primitive('d3m.primitives.tods.detection_algorithm.AutoRegODetector'))
step_4.add_argument(name='inputs', argument_type=ArgumentType.CONTAINER, data_reference='steps.2.produce')
SIZE = 10
step_4.add_hyperparameter(name='contamination', argument_type=ArgumentType.VALUE, data=0.25)
step_4.add_hyperparameter(name='window_size', argument_type=ArgumentType.VALUE, data=SIZE)
...
with open('ar_pipeline_win' + str(SIZE) + '.json', 'w') as f:
    f.write(data)
    print(data)
