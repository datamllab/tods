...
parser.add_argument('--pipeline_path', 
                    default=os.path.join(this_path, 'mogaal_pipeline_k7_con0.2.json'),
                    help='Input the path of the pre-built pipeline description')

result_container = []
args = parser.parse_args()

df = pd.read_csv(os.path.join(this_path, '../multidataset/0123.csv'))
...
df = pd.read_csv(os.path.join(this_path, '../multidataset/1234.csv'))
dataset = generate_dataset(df, args.target_index)
pipeline = load_pipeline(args.pipeline_path)
