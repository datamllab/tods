...
class TestRandomSearch(unittest.TestCase):
    def setUp(self):
        ...
        self.tuner_base = RandomSearch(problem, backend=backend, max_trials=10, directory=self.test_dir)
    ...
    def test_fit_ray(self):
        ...
        tuner_base = RandomSearch(self.problem, backend=backend, max_trials=30, directory=self.test_dir)
        _, pipeline_result = tuner_base.search_fit(input_data=[self.dataset], time_limit=60)
        self.assertEqual(pipeline_result.error, None)
        ray.shutdown()
...
if __name__ == '__main__':
    ...
    for test_case in (
        ...
        suite.addTest(TestRandomSearch(test_case))
