...
class SimpleRunnerTestCase(unittest.TestCase):
    ...
    def test_fit_produce_pipelines(self):
        ...
        simple_runner = SimpleRunner(random_seed=42, volumes_dir=None, scratch_dir=self.test_dir)
        result = simple_runner.fit_pipeline(problem_description=problem_description, pipeline=pipeline,
                                            input_data=[dataset])
        self.assertEqual(result.status, 'COMPLETED')

        result = simple_runner.produce_pipeline(fitted_pipeline_id=result.fitted_pipeline_id, input_data=[dataset])
        self.assertEqual(result.status, 'COMPLETED')
    ...
    def test_evaluate_pipelines(self):
        ...
        result = simple_runner.evaluate_pipeline(
