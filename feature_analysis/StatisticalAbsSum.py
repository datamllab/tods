...
class StatisticalAbsSumPrimitive(transformer.TransformerPrimitiveBase[Inputs, Outputs, Hyperparams]):
    ...
    def produce(self, *, inputs: Inputs, timeout: float = None, iterations: int = None) -> base.CallResult[Outputs]:
        ...
        output_columns = []
        if len(self._training_indices) > 0:
            statistical_abs_sum_output = self._abs_sum(statistical_abs_sum_input,self.hyperparams["window_size"])

            if sparse.issparse(statistical_abs_sum_output):
                statistical_abs_sum_output = statistical_abs_sum_output.toarray()
            outputs = self._wrap_predictions(inputs, statistical_abs_sum_output)
    ...
    def _abs_sum(self,X,window_size):
        """ statistical abs_sum of time series sequence
           Args:
            X : DataFrame
               Time series.
        Returns:
            DataFrame
            A object with abs_sum
        """
        if(window_size==-1):
            window_size = len(X)
        ...
        for column in X.columns:
            ...
            column_abs_sum = np.zeros(len(column_value))
            for iter in range(window_size-1,len(column_value)):
                sequence = column_value[iter-window_size+1:iter+1]
                column_abs_sum[iter] = np.sum(np.abs(sequence))
            column_abs_sum[:window_size-1] = column_abs_sum[window_size-1]
            transformed_X[column + "_abs_sum"] = column_abs_sum
