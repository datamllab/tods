...
class StatisticalZeroCrossingPrimitive(transformer.TransformerPrimitiveBase[Inputs, Outputs, Hyperparams]):
    ...
    def produce(self, *, inputs: Inputs, timeout: float = None, iterations: int = None) -> base.CallResult[Outputs]:
        ...
        Returns:
            Container DataFrame containing zero crossing indicator of  time series
        """
        self.logger.info('Statistical ZeroCrossing  Primitive called')
        ...
        statistical_zero_crossing_input = inputs
        if self.hyperparams['use_semantic_types']:
            statistical_zero_crossing_input = inputs.iloc[:, self._training_indices]
        output_columns = []
        if len(self._training_indices) > 0:
            statistical_zero_crossing_output = self._zero_crossing(statistical_zero_crossing_input)

            if sparse.issparse(statistical_zero_crossing_output):
                statistical_zero_crossing_output = statistical_zero_crossing_output.toarray()
            outputs = self._wrap_predictions(inputs, statistical_zero_crossing_output)
        ...
        self.logger.info('Statistical ZeroCrossing  Primitive returned')
    ...
    def _zero_crossing(self,X):
        """ statistical zero crossing indicatoe of time series sequence
           Args:
            X : DataFrame
               Time series.
        Returns:
            DataFrame
            A object with zero crossing
        """
        transformed_X = utils.pandas.DataFrame()
        for column in X.columns:
            column_value = X[column].values
            column_zero_crossing = np.zeros(len(column_value))
            for iter in range(1,len(column_value)):
                if(column_value[iter]*column_value[iter-1] < 0.0 ):
                    column_zero_crossing[iter] = 1
            transformed_X[column + "_zero_crossing"] = column_zero_crossing
