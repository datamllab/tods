...
from tods.detection_algorithm.core.mad_gan.mmd import rbf_mmd2, median_pairwise_distance, mix_rbf_mmd2_and_ratio
...
class MADGAN(BaseDetector):
    ...
    def _build_model(self):
        ...
        # --- build model --- #
        batch_size = self.MG_hyperparams['batch_size']
        seq_length = self.MG_hyperparams['seq_length']
        latent_dim = self.MG_hyperparams['latent_dim']
        #num_variables = self.MG_hyperparams['num_variables']
        ...
        learning_rate = self.MG_hyperparams['learning_rate']
        l2norm_bound = self.MG_hyperparams['l2norm_bound']
        batches_per_lot = self.MG_hyperparams['batches_per_lot']
        dp_sigma = self.MG_hyperparams['dp_sigma']
        dp = self.MG_hyperparams['dp']
        ...
        return samples, identifier, settings, num_variables, labels, Z, X, T, D_loss, G_loss, D_solver, G_solver
    ...
    def fit(self,X,y=None):
        ...
        samples, identifier, settings, num_variables, labels, Z, X, T, D_loss, G_loss, D_solver, G_solver = self._build_model()
        ...
        num_epochs = self.MG_hyperparams['num_epochs'] 
