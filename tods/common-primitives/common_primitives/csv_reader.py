...
class CSVReaderPrimitive(base.FileReaderPrimitiveBase):
    """
    ...
    metadata = metadata_base.PrimitiveMetadata(
        {
            ...
            'python_path': 'd3m.primitives.data_preprocessing.csv_reader.Common',
            'keywords': ['CSV', 'reader'],
            'source': {
    ...
    def _read_fileuri(self, metadata: frozendict.FrozenOrderedDict, fileuri: str) -> container.DataFrame:
        ...
        expected_names = None
        if metadata.get('file_columns', None):
            expected_names = []
            for column in metadata['file_columns']:
                expected_names.append(column['name'])
    ...
    def _produce_column_metadata(self, inputs_metadata: metadata_base.DataMetadata, column_index: int,
        ...
        if column_meta.get('file_columns', None):
            for i, column in enumerate(column_meta['file_columns']):
                ...
                column_metadata = column_metadata.update((metadata_base.ALL_ELEMENTS, 0, metadata_base.ALL_ELEMENTS, i), {'structural_type': str})
