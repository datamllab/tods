...
from common_primitives import dataset_to_dataframe, extract_columns_semantic_types, xgboost_gbtree, column_parser
...
class XGBoostTestCase(unittest.TestCase):
    ...
    def test_single_target(self):
        ...
        hyperparams_class = \
            xgboost_gbtree.XGBoostGBTreeClassifierPrimitive.metadata.query()['primitive_code']['class_type_arguments'][
                'Hyperparams']
        primitive = xgboost_gbtree.XGBoostGBTreeClassifierPrimitive(
        ...
        self.assertAlmostEqual(log_likelihood.iloc[0, 0], -3.4919378757476807)
        self.assertEqual(log_likelihoods.metadata.query_column(0)['name'], 'species')
    ...
    def test_single_target_continue_fit(self):
        ...
        self.assertEqual(log_likelihood.shape, (1, 1))
        self.assertAlmostEqual(log_likelihood.iloc[0, 0], -2.4149818420410156)
        self.assertEqual(log_likelihoods.metadata.query_column(0)['name'], 'species')
    ...
    def test_multiple_targets(self):
        ...
        for column_index in range(2):
            self.assertAlmostEqual(log_likelihood.iloc[0, column_index], -3.4919378757476807)
            self.assertEqual(log_likelihoods.metadata.query_column(column_index)['name'], 'species')
        ...
        self.assertEqual(feature_importances.values.tolist(), [[0.012397459708154202,
            0.03404613956809044,
            0.5992223024368286,
            0.35433411598205566,
        ]])
    ...
    def test_multiple_targets_continue_fit(self):
        ...
        self.assertAlmostEqual(log_likelihood.iloc[0, 0], -2.4149818420410156)
        self.assertEqual(log_likelihoods.metadata.query_column(0)['name'], 'species')
        self.assertAlmostEqual(log_likelihood.iloc[0, 1], -2.4149818420410156)
        self.assertEqual(log_likelihoods.metadata.query_column(1)['name'], 't-species')

        feature_importances = primitive.produce_feature_importances().value

        self.assertEqual(feature_importances.values.tolist(),
            [[0.011062598787248135,
    ...
    def test_return_append(self):
        ...
        hyperparams_class = \
            xgboost_gbtree.XGBoostGBTreeClassifierPrimitive.metadata.query()['primitive_code']['class_type_arguments'][
                'Hyperparams']
        primitive = xgboost_gbtree.XGBoostGBTreeClassifierPrimitive(hyperparams=hyperparams_class.defaults())
    ...
    def test_pickle_unpickle(self):
        ...
        # try to pickle again to see if we load it properly
