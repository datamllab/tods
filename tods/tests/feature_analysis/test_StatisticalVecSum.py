...
class StatisticalVecSumTestCase(unittest.TestCase):
    def test_basic(self):
        self.maxDiff=None
        ...
        hyperparams_class = StatisticalVecSum.StatisticalVecSumPrimitive.metadata.get_hyperparams()
        ...
        expected_output = container.DataFrame(
            {'timestamp': [1, 3, 2, 5], 'values': [1.0, 2.0, 3.0, 4.0], 'b': [1.0, 4.0, -5.0, 6.0],
             'values_vec_sum': [3.0,3.0,5.0,7.0], 'b_vec_sum': [5.0,5.0,-1.0,1.0]},
            columns=['timestamp', 'values', 'b', 'values_vec_sum', 'b_vec_sum'])

        self.assertEqual(output_main[['timestamp', 'values', 'b', 'values_vec_sum',
                                      'b_vec_sum']].values.tolist(), expected_output[
                             ['timestamp', 'values', 'b', 'values_vec_sum', 'b_vec_sum'
                              ]].values.tolist())
