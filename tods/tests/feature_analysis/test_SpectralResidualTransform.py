...
class SpectralResidualTransformTestCase(unittest.TestCase):
    def test_basic(self):
        self.maxDiff=None
        ...
        hyperparams_class = SpectralResidualTransform.SpectralResidualTransformPrimitive.metadata.get_hyperparams()
        ...
        expected_output = container.DataFrame(
            {'timestamp': [1, 3, 2, 5], 'values': [1.0, 2.0, 3.0, 4.0], 'b': [1.0, 4.0, 5.0, 6.0],
             'values_spectral_residual': [0.2018, 0.1364, 0.4252, 0.6807], 'b_spectral_residual': [0.2584, 0.2536, 0.4522, 0.5880]},
            columns=['timestamp', 'values', 'b', 'values_spectral_residual', 'b_spectral_residual'])

        self.assertEqual(output_main[['timestamp', 'values', 'b', 'values_spectral_residual',
                                      'b_spectral_residual']].values.tolist(), expected_output[
                             ['timestamp', 'values', 'b', 'values_spectral_residual', 'b_spectral_residual'
                              ]].values.tolist())
        ...
        self.assertEqual(utils.to_json_structure(output_main.metadata.to_internal_simple_structure()),
                         [{'metadata': {'dimension': {'length': 4,
                          ...
                          {'metadata': {'name': 'b', 'structural_type': 'numpy.float64'},
                           'selector': ['__ALL_ELEMENTS__', 2]},
                          {'metadata': {'name': 'values_spectral_residual',
                                        'semantic_types': ['https://metadata.datadrivendiscovery.org/types/Attribute'],
                                        'structural_type': 'numpy.float64'},
                           'selector': ['__ALL_ELEMENTS__', 3]},
                          {'metadata': {'name': 'b_spectral_residual',
                                        'semantic_types': ['https://metadata.datadrivendiscovery.org/types/Attribute'],
                                        'structural_type': 'numpy.float64'},
