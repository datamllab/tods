...
class StatisticalKurtosisTestCase(unittest.TestCase):
    def test_basic(self):
        self.maxDiff=None
        ...
        hyperparams_class = StatisticalKurtosis.StatisticalKurtosisPrimitive.metadata.get_hyperparams()
        ...
        expected_output = container.DataFrame(
            {'timestamp': [1, 3, 2, 5], 'values': [1.0, 2.0, 3.0, 4.0], 'b': [1.0, 4.0, 5.0, 6.0],
             'values_kurtosis': [-2.0,-2.0,-2.0,-2.0], 'b_kurtosis': [-2.0,-2.0,-2.0,-2.0]},
            columns=['timestamp', 'values', 'b', 'values_kurtosis', 'b_kurtosis'])

        self.assertEqual(output_main[['timestamp', 'values', 'b', 'values_kurtosis',
                                      'b_kurtosis']].values.tolist(), expected_output[
                             ['timestamp', 'values', 'b', 'values_kurtosis', 'b_kurtosis'
                              ]].values.tolist())
