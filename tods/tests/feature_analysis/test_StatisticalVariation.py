...
class StatisticalVariationTestCase(unittest.TestCase):
    def test_basic(self):
        ...
        hyperparams_class = StatisticalVariation.StatisticalVariationPrimitive.metadata.get_hyperparams()
        ...
        expected_output = container.DataFrame(
            {'timestamp': [1, 3, 2, 5], 'values': [1.0, 2.0, 3.0, 4.0], 'b': [1.0, 4.0, 5.0, 6.0],
             'values_variation': [0.3333, 0.3333, 0.2000, 0.1429], 'b_variation': [0.6000, 0.6000, 0.1111, 0.0909]},
            columns=['timestamp', 'values', 'b', 'values_variation', 'b_variation'])

        self.assertEqual(output_main[['timestamp', 'values', 'b', 'values_variation',
                                      'b_variation']].values.tolist(), expected_output[
                             ['timestamp', 'values', 'b', 'values_variation', 'b_variation'
                              ]].values.tolist())
