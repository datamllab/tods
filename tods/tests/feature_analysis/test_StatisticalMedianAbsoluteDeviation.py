...
class StatisticalMedianAbsoluteDeviationTestCase(unittest.TestCase):
    def test_basic(self):
        self.maxDiff=None
        ...
        hyperparams_class = StatisticalMedianAbsoluteDeviation.StatisticalMedianAbsoluteDeviationPrimitive.metadata.get_hyperparams()
        ...
        expected_output = container.DataFrame(
            {'timestamp': [1, 3, 2, 5], 'values': [1.0, 2.0, 3.0, 4.0], 'b': [1.0, 4.0, 5.0, 6.0],
             'values_median_absolute_deviation': [0.7413, 0.7413, 0.7413, 0.7413], 'b_median_absolute_deviation': [2.2239, 2.2239, 0.7413, 0.7413]},
            columns=['timestamp', 'values', 'b', 'values_median_absolute_deviation', 'b_median_absolute_deviation'])

        self.assertEqual(output_main[['timestamp', 'values', 'b', 'values_median_absolute_deviation',
                                      'b_median_absolute_deviation']].values.tolist(), expected_output[
                             ['timestamp', 'values', 'b', 'values_median_absolute_deviation', 'b_median_absolute_deviation'
                              ]].values.tolist())
        ...
        self.assertEqual(utils.to_json_structure(output_main.metadata.to_internal_simple_structure()),
                         [{'metadata': {'dimension': {'length': 4,
                          ...
                          {'metadata': {'name': 'b', 'structural_type': 'numpy.float64'},
                           'selector': ['__ALL_ELEMENTS__', 2]},
                          {'metadata': {'name': 'values_median_absolute_deviation',
                                        'semantic_types': ['https://metadata.datadrivendiscovery.org/types/Attribute'],
                                        'structural_type': 'numpy.float64'},
                           'selector': ['__ALL_ELEMENTS__', 3]},
                          {'metadata': {'name': 'b_median_absolute_deviation',
                                        'semantic_types': ['https://metadata.datadrivendiscovery.org/types/Attribute'],
                                        'structural_type': 'numpy.float64'},
