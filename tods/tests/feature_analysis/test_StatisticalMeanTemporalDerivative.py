...
class StatisticalMeanTemporalDerivativeTestCase(unittest.TestCase):
    def test_basic(self):
        self.maxDiff=None
        ...
        hyperparams_class = StatisticalMeanTemporalDerivative.StatisticalMeanTemporalDerivativePrimitive.metadata.get_hyperparams()
        ...
        expected_output = container.DataFrame(
            {'timestamp': [1, 3, 2, 5], 'values': [1.0, 2.0, 3.0, 4.0], 'b': [1.0, 4.0, 5.0, 6.0],
             'values_mean_temporal_derivative': [1.0, 1.0, 1.0, 1.0], 'b_mean_temporal_derivative': [3.0, 3.0, 1.0, 1.0]},
            columns=['timestamp', 'values', 'b', 'values_mean_temporal_derivative', 'b_mean_temporal_derivative'])

        self.assertEqual(output_main[['timestamp', 'values', 'b', 'values_mean_temporal_derivative',
                                      'b_mean_temporal_derivative']].values.tolist(), expected_output[
        ...
        self.assertEqual(utils.to_json_structure(output_main.metadata.to_internal_simple_structure()),
                         [{'metadata': {'dimension': {'length': 4,
                          ...
                          {'metadata': {'name': 'b', 'structural_type': 'numpy.float64'},
                           'selector': ['__ALL_ELEMENTS__', 2]},
                          {'metadata': {'name': 'values_mean_temporal_derivative',
                                        'semantic_types': ['https://metadata.datadrivendiscovery.org/types/Attribute'],
                                        'structural_type': 'numpy.float64'},
                           'selector': ['__ALL_ELEMENTS__', 3]},
                          {'metadata': {'name': 'b_mean_temporal_derivative',
                                        'semantic_types': ['https://metadata.datadrivendiscovery.org/types/Attribute'],
                                        'structural_type': 'numpy.float64'},
