...
class StatisticalHmeanTestCase(unittest.TestCase):
    def test_basic(self):
        self.maxDiff=None
        ...
        hyperparams_class = StatisticalHmean.StatisticalHmeanPrimitive.metadata.get_hyperparams()
        ...
        expected_output = container.DataFrame(
            {'timestamp': [1, 3, 2, 5], 'values': [1.0, 2.0, 3.0, 4.0], 'b': [1.0, 4.0, 5.0, 6.0],
             'values_hmean': [1.3333, 1.3333, 2.4000, 3.4286], 'b_hmean': [1.6000, 1.6000, 4.4444, 5.4545]},
            columns=['timestamp', 'values', 'b', 'values_hmean', 'b_hmean'])

        self.assertEqual(output_main[['timestamp', 'values', 'b', 'values_hmean',
                                      'b_hmean']].values.tolist(), expected_output[
                             ['timestamp', 'values', 'b', 'values_hmean', 'b_hmean'
                              ]].values.tolist())
