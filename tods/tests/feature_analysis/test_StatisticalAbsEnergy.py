...
class StatisticalAbsEnergyTestCase(unittest.TestCase):
    def test_basic(self):
        self.maxDiff=None
        ...
        hyperparams_class = StatisticalAbsEnergy.StatisticalAbsEnergyPrimitive.metadata.get_hyperparams()
        ...
        expected_output = container.DataFrame(
            {'timestamp': [1, 3, 2, 5], 'values': [1.0, 2.0, 3.0, 4.0], 'b': [1.0, 4.0, 5.0, 6.0],
             'values_abs_energy': [5.0,5.0, 13.0, 25.0], 'b_abs_energy': [17.0,17.0, 41.0,61.0]},
            columns=['timestamp', 'values', 'b', 'values_abs_energy', 'b_abs_energy'])

        self.assertEqual(output_main[['timestamp', 'values', 'b', 'values_abs_energy',
                                      'b_abs_energy']].values.tolist(), expected_output[
                             ['timestamp', 'values', 'b', 'values_abs_energy', 'b_abs_energy'
                              ]].values.tolist())
