...
class StatisticalVarTestCase(unittest.TestCase):
    def test_basic(self):
        self.maxDiff=None
        ...
        hyperparams_class = StatisticalVar.StatisticalVarPrimitive.metadata.get_hyperparams()
        ...
        expected_output = container.DataFrame(
            {'timestamp': [1, 3, 2, 5], 'values': [1.0, 2.0, 3.0, 4.0], 'b': [1.0, 4.0, 5.0, 6.0],
             'values_var': [0.25, 0.25, 0.25, 0.25], 'b_var': [2.25, 2.25, 0.25, 0.25]},
            columns=['timestamp', 'values', 'b', 'values_var', 'b_var'])

        self.assertEqual(output_main[['timestamp', 'values', 'b', 'values_var',
                                      'b_var']].values.tolist(), expected_output[
                             ['timestamp', 'values', 'b', 'values_var', 'b_var'
                              ]].values.tolist())
