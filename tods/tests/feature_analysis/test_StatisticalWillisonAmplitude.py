...
class StatisticalWillisonAmplitudeTestCase(unittest.TestCase):
    def test_basic(self):
        self.maxDiff=None
        ...
        hyperparams_class = StatisticalWillisonAmplitude.StatisticalWillisonAmplitudePrimitive.metadata.get_hyperparams()
        ...
        expected_output = container.DataFrame(
            {'timestamp': [1, 3, 2, 5], 'values': [1.0, 2.0, 3.0, 4.0], 'b': [1.0, 4.0, 5.0, 6.0],
             'values_willison_amplitude': [0.0,0.0,0.0,0.0], 'b_willison_amplitude': [1.0,1.0,1.0,0.0]},
            columns=['timestamp', 'values', 'b', 'values_willison_amplitude', 'b_willison_amplitude'])

        self.assertEqual(output_main[['timestamp', 'values', 'b', 'values_willison_amplitude',
                                      'b_willison_amplitude']].values.tolist(), expected_output[
                             ['timestamp', 'values', 'b', 'values_willison_amplitude', 'b_willison_amplitude'
                              ]].values.tolist())
        ...
        self.assertEqual(utils.to_json_structure(output_main.metadata.to_internal_simple_structure()),
                         [{'metadata': {'dimension': {'length': 4,
                          ...
                          {'metadata': {'name': 'b', 'structural_type': 'numpy.float64'},
                           'selector': ['__ALL_ELEMENTS__', 2]},
                          {'metadata': {'name': 'values_willison_amplitude',
