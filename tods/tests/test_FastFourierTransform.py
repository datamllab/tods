...
class FftTestCase(unittest.TestCase):
    def test_basic(self):
        ...
        hyperparams_class = FastFourierTransform.FastFourierTransform.metadata.get_hyperparams()
        hp = hyperparams_class.defaults().replace({
            'use_semantic_types':True,
            'use_columns': (0,),
            'return_result':'append',
        })
        primitive = FastFourierTransform.FastFourierTransform(hyperparams=hp)
