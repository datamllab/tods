...
.. dropdown:: Control Parameter
    ...
    metadata = construct_primitive_metadata(module='feature_analysis', name='statistical_variation', id='StatisticalVariationPrimitive', primitive_family='feature_construct', hyperparams=['window_size'], description='Time Series Decompostional')
    ...
    def _produce(self, *, inputs: Inputs, timeout: float = None, iterations: int = None) -> base.CallResult[Outputs]:
        ...
        Returns:
            Container DataFrame containing variation of  time series
        """
        self.logger.info('Statistical Variation  Primitive called')
        ...
        statistical_variation_input = inputs
        if self.hyperparams['use_semantic_types']:
            statistical_variation_input = inputs.iloc[:, self._training_indices]
        output_columns = []
        if len(self._training_indices) > 0:
            statistical_variation_output = self._variation(statistical_variation_input,self.hyperparams["window_size"])

            if sparse.issparse(statistical_variation_output):
                statistical_variation_output = statistical_variation_output.toarray()
            outputs = self._wrap_predictions(inputs, statistical_variation_output)
        ...
        self.logger.info('Statistical Variation  Primitive returned')
    ...
    def _variation(self,X,window_size):
        """ statistical variation of time series sequence
           Args:
            X : DataFrame
               Time series.
        Returns:
            DataFrame
            A object with variation
        """
        if(window_size==-1):
            window_size = len(X)
        ...
        for column in X.columns:
            ...
            column_variation = np.zeros(len(column_value))
            for iter in range(window_size-1,len(column_value)):
                sequence = column_value[iter-window_size+1:iter+1]
                column_variation[iter] = stats.variation(sequence).round(4)
            column_variation[:window_size-1] = column_variation[window_size-1]
            transformed_X[column + "_variation"] = column_variation
