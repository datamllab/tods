...
__all__ = ('StatisticalAbsEnergyPrimitive',)
...
class StatisticalAbsEnergyPrimitive(transformer.TransformerPrimitiveBase[Inputs, Outputs, Hyperparams]):
    ...
    def produce(self, *, inputs: Inputs, timeout: float = None, iterations: int = None) -> base.CallResult[Outputs]:
        ...
        if self.hyperparams['use_semantic_types']:
            statistical_abs_energy_input = inputs.iloc[:, self._training_indices]
        output_columns = []
        if len(self._training_indices) > 0:
            statistical_abs_energy_output = self._abs_energy(statistical_abs_energy_input,self.hyperparams["window_size"])

            if sparse.issparse(statistical_abs_energy_output):
                statistical_abs_energy_output = statistical_abs_energy_output.toarray()
            outputs = self._wrap_predictions(inputs, statistical_abs_energy_output)
    ...
    def _abs_energy(self,X,window_size):
        ...
        for column in X.columns:
            ...
            for iter in range(window_size-1,len(column_value)):
                ...
                column_abs_energy[iter] = np.round(np.sum(sequence*sequence),4)
            column_abs_energy[:window_size-1] = column_abs_energy[window_size-1]
