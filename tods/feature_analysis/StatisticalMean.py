...
class StatisticalMeanPrimitive(transformer.TransformerPrimitiveBase[Inputs, Outputs, Hyperparams]):
    ...
    def produce(self, *, inputs: Inputs, timeout: float = None, iterations: int = None) -> base.CallResult[Outputs]:
        ...
        """
        self.logger.info('Statistical Mean  Primitive called')
        ...
        statistical_mean_input = inputs
        if self.hyperparams['use_semantic_types']:
            statistical_mean_input = inputs.iloc[:, self._training_indices]
        output_columns = []
        if len(self._training_indices) > 0:
            statistical_mean_output = self._mean(statistical_mean_input,self.hyperparams["window_size"])

            if sparse.issparse(statistical_mean_output):
                statistical_mean_output = statistical_mean_output.toarray()
            outputs = self._wrap_predictions(inputs, statistical_mean_output)
        ...
        self.logger.info('Statistical Mean  Primitive returned')
    ...
    def _mean(self,X,window_size):
        """ statistical mean of time series sequence
           Args:
            X : DataFrame
               Time series.
        Returns:
        ...
        for column in X.columns:
            ...
            column_mean = np.zeros(len(column_value))
            for iter in range(window_size-1,len(column_value)):
                sequence = column_value[iter-window_size+1:iter+1]
                column_mean[iter] = np.mean(sequence)
            column_mean[:window_size-1] = column_mean[window_size-1]
            transformed_X[column + "_mean"] = column_mean
