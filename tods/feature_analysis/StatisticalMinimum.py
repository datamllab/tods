...
.. dropdown:: Control Parameter
    ...
    metadata = construct_primitive_metadata(module='feature_analysis', name='statistical_minimum', id='StatisticalMinimumPrimitive', primitive_family='feature_construct', hyperparams=['window_size'], description='Time Series Decompostional')
    ...
    def _produce(self, *, inputs: Inputs, timeout: float = None, iterations: int = None) -> base.CallResult[Outputs]:
        ...
        statistical_minimum_input = inputs
        if self.hyperparams['use_semantic_types']:
            statistical_minimum_input = inputs.iloc[:, self._training_indices]
        output_columns = []
        if len(self._training_indices) > 0:
            statistical_minimum_output = self._minimum(statistical_minimum_input,self.hyperparams["window_size"])

            if sparse.issparse(statistical_minimum_output):
                statistical_minimum_output = statistical_minimum_output.toarray()
            outputs = self._wrap_predictions(inputs, statistical_minimum_output)
    ...
    def _minimum(self,X,window_size):
        """ statistical minimum of time series sequence
           Args:
            X : DataFrame
               Time series.
        Returns:
            DataFrame
            A object with minimum
        """
        if(window_size==-1):
            window_size = len(X)
        ...
        for column in X.columns:
            ...
            column_minimum = np.zeros(len(column_value))
            for iter in range(window_size-1,len(column_value)):
                sequence = column_value[iter-window_size+1:iter+1]
                column_minimum[iter] = np.min(sequence)
            column_minimum[:window_size-1] = column_minimum[window_size-1]
            transformed_X[column + "_minimum"] = column_minimum
