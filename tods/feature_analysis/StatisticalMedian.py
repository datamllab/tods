...
class Hyperparams(hyperparams.Hyperparams):
       ...
       #Tuning Parameter
       #default windiow -1 considers entire time series is considered
...
.. dropdown:: Control Parameter
    ...
    """
    metadata = construct_primitive_metadata(module='feature_analysis', name='statistical_median', id='StatisticalMedianPrimitive', primitive_family='feature_construct', hyperparams=['window_size'], description='Time Series Decompostional')
    ...
    def _produce(self, *, inputs: Inputs, timeout: float = None, iterations: int = None) -> base.CallResult[Outputs]:
        ...
        Returns:
            Container DataFrame containing median of  time series
        """
        self.logger.info('Statistical Median  Primitive called')
        ...
        statistical_median_input = inputs
        if self.hyperparams['use_semantic_types']:
            statistical_median_input = inputs.iloc[:, self._training_indices]
        output_columns = []
        if len(self._training_indices) > 0:
            statistical_median_output = self._median(statistical_median_input,self.hyperparams["window_size"])

            if sparse.issparse(statistical_median_output):
                statistical_median_output = statistical_median_output.toarray()
            outputs = self._wrap_predictions(inputs, statistical_median_output)
        ...
        self.logger.info('Statistical Median  Primitive returned')
    ...
    def _median(self,X,window_size):
        """ statistical median of time series sequence
           Args:
            X : DataFrame
               Time series.
        Returns:
            DataFrame
            A object with median
        """
        if(window_size==-1):
            window_size = len(X)
        ...
        for column in X.columns:
            ...
            column_median = np.zeros(len(column_value))
            for iter in range(window_size-1,len(column_value)):
                sequence = column_value[iter-window_size+1:iter+1]
                column_median[iter] = np.median(sequence)
            column_median[:window_size-1] = column_median[window_size-1]
            transformed_X[column + "_median"] = column_median
