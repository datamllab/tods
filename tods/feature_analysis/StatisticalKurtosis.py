...
class StatisticalKurtosisPrimitive(transformer.TransformerPrimitiveBase[Inputs, Outputs, Hyperparams]):
    ...
    def produce(self, *, inputs: Inputs, timeout: float = None, iterations: int = None) -> base.CallResult[Outputs]:
        ...
        Returns:
            Container DataFrame containing kurtosis of  time series
        """
        self.logger.info('Statistical Kurtosis  Primitive called')
        ...
        statistical_kurtosis_input = inputs
        if self.hyperparams['use_semantic_types']:
            statistical_kurtosis_input = inputs.iloc[:, self._training_indices]
        output_columns = []
        if len(self._training_indices) > 0:
            statistical_kurtosis_output = self._kurtosis(statistical_kurtosis_input,self.hyperparams["window_size"])

            if sparse.issparse(statistical_kurtosis_output):
                statistical_kurtosis_output = statistical_kurtosis_output.toarray()
            outputs = self._wrap_predictions(inputs, statistical_kurtosis_output)
        ...
        self.logger.info('Statistical Kurtosis  Primitive returned')
    ...
    def _kurtosis(self,X,window_size):
        """ statistical kurtosis of time series sequence
           Args:
            X : DataFrame
               Time series.
        Returns:
            DataFrame
            A object with kurtosis
        """
        if(window_size==-1):
            window_size = len(X)
        ...
        for column in X.columns:
            ...
            column_kurtosis = np.zeros(len(column_value))
            for iter in range(window_size-1,len(column_value)):
                sequence = column_value[iter-window_size+1:iter+1]
                column_kurtosis[iter] = round(stats.kurtosis(sequence),4)
            column_kurtosis[:window_size-1] = column_kurtosis[window_size-1]
            transformed_X[column + "_kurtosis"] = column_kurtosis
