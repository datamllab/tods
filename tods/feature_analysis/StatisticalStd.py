...
.. dropdown:: Control Parameter
    ...
    metadata = construct_primitive_metadata(module='feature_analysis', name='statistical_std', id='StatisticalMeanAbsTemporalDerivativePrimitive', primitive_family='feature_construct', hyperparams=['window_size'], description='Time Series Decompostional')
    ...
    def _produce(self, *, inputs: Inputs, timeout: float = None, iterations: int = None) -> base.CallResult[Outputs]:
        ...
        Returns:
            Container DataFrame containing std of  time series
        """
        self.logger.info('Statistical Std  Primitive called')
        ...
        statistical_std_input = inputs
        if self.hyperparams['use_semantic_types']:
            statistical_std_input = inputs.iloc[:, self._training_indices]
        output_columns = []
        if len(self._training_indices) > 0:
            statistical_std_output = self._std(statistical_std_input,self.hyperparams["window_size"])

            if sparse.issparse(statistical_std_output):
                statistical_std_output = statistical_std_output.toarray()
            outputs = self._wrap_predictions(inputs, statistical_std_output)
        ...
        self.logger.info('Statistical Std  Primitive returned')
    ...
    def _std(self,X,window_size):
        """ statistical std of time series sequence
           Args:
            X : DataFrame
               Time series.
        Returns:
        ...
        for column in X.columns:
            ...
            column_std = np.zeros(len(column_value))
            for iter in range(window_size-1,len(column_value)):
                sequence = column_value[iter-window_size+1:iter+1]
                column_std[iter] = np.std(sequence)
            column_std[:window_size-1] = column_std[window_size-1]
            transformed_X[column + "_std"] = column_std
