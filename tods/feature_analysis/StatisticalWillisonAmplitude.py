...
class Hyperparams(hyperparams.Hyperparams):
       ...
       threshold = hyperparams.Hyperparameter(default= 0, semantic_types=[
           'https://metadata.datadrivendiscovery.org/types/TuningParameter',
       ], description="threshold for willison amplitude")
       #control parameter
...
.. dropdown:: Control Parameter
    ...
    metadata = construct_primitive_metadata(module='feature_analysis', name='statistical_willison_amplitude', id='StatisticalWillisonAmplitudePrimitive', primitive_family='feature_construct', hyperparams=['window_size'], description='Time Series Decompostional')
    ...
    def _produce(self, *, inputs: Inputs, timeout: float = None, iterations: int = None) -> base.CallResult[Outputs]:
        ...
        Returns:
            Container DataFrame containing willison amplitude of  time series
        """
        self.logger.info('Statistical Willison Amplitude  Primitive called')
        ...
        statistical_willison_amplitude_input = inputs
        if self.hyperparams['use_semantic_types']:
            statistical_willison_amplitude_input = inputs.iloc[:, self._training_indices]
        output_columns = []
        if len(self._training_indices) > 0:
            statistical_willison_amplitude_output = self._willison_amplitude(statistical_willison_amplitude_input,
                                                                             self.hyperparams["window_size"],self.hyperparams['threshold'])

            if sparse.issparse(statistical_willison_amplitude_output):
                statistical_willison_amplitude_output = statistical_willison_amplitude_output.toarray()
            outputs = self._wrap_predictions(inputs, statistical_willison_amplitude_output)
        ...
        self.logger.info('Statistical Willison Amplitude  Primitive returned')
    ...
    def _willison_amplitude(self,X,window_size,threshold):
        """ statistical willison_amplitude of time series sequence
           Args:
            X : DataFrame
               Time series.
        Returns:
            DataFrame
            A object with willison_amplitude
        """
        if(window_size==-1):
            window_size = len(X)
        transformed_X = utils.pandas.DataFrame()
        for column in X.columns:
            column_diff_value = np.zeros(len(X[column])+1)    # array of 1 extra row use to adjust X(t)-X(t-1)
            column_diff_value[1:] = (X[column].values)
            column_diff_value = np.abs(np.diff(column_diff_value))
            willison_func_values = np.zeros(len(X[column]))
            mask = (column_diff_value>threshold)
            willison_func_values[mask] = 1

            column_willison_amplitude = np.zeros(len(X[column].values))

            for iter in range(window_size-1,len(column_diff_value)):
                sequence = willison_func_values[iter-window_size+1:iter+1]
                column_willison_amplitude[iter] = np.sum(sequence)
            column_willison_amplitude[:window_size-1] = column_willison_amplitude[window_size-1]
            transformed_X[column + "_willison_amplitude"] = column_willison_amplitude
