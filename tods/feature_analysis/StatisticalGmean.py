...
class StatisticalGmeanPrimitive(transformer.TransformerPrimitiveBase[Inputs, Outputs, Hyperparams]):
    ...
    def produce(self, *, inputs: Inputs, timeout: float = None, iterations: int = None) -> base.CallResult[Outputs]:
        ...
        Returns:
            Container DataFrame containing gmean of  time series
        """
        self.logger.info('Statistical Gmean  Primitive called')
        ...
        statistical_gmean_input = inputs
        if self.hyperparams['use_semantic_types']:
            statistical_gmean_input = inputs.iloc[:, self._training_indices]
        output_columns = []
        if len(self._training_indices) > 0:
            statistical_gmean_output = self._gmean(statistical_gmean_input,self.hyperparams["window_size"])


            if sparse.issparse(statistical_gmean_output):
                statistical_gmean_output = statistical_gmean_output.toarray()
            outputs = self._wrap_predictions(inputs, statistical_gmean_output)
        ...
        self.logger.info('Statistical Gmean  Primitive returned')
    ...
    def _gmean(self,X,window_size):
        """ statistical gmean of time series sequence
           Args:
            X : DataFrame
               Time series.
        Returns:
        ...
        for column in X.columns:
            ...
            column_gmean = np.zeros(len(column_value))
            for iter in range(window_size-1,len(column_value)):
                sequence = column_value[iter-window_size+1:iter+1]
                column_gmean[iter] = stats.gmean(sequence).round(4)
            column_gmean[:window_size-1] = column_gmean[window_size-1]
            transformed_X[column + "_gmean"] = column_gmean
