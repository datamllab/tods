...
.. dropdown:: Control Parameter 
    ...
    """
    metadata = construct_primitive_metadata(module='feature_analysis', name='statistical_mean_abs', id='StatisticalMeanAbsPrimitive', primitive_family='feature_construct', hyperparams=['window_size'], description='Time Series Decompostional')
    ...
    def _produce(self, *, inputs: Inputs, timeout: float = None, iterations: int = None) -> base.CallResult[Outputs]:
        ...
        Returns:
            Container DataFrame containing mean_abs of  time series
        """
        self.logger.info('Statistical MeanAbs  Primitive called')
        ...
        statistical_mean_abs_input = inputs
        if self.hyperparams['use_semantic_types']:
            statistical_mean_abs_input = inputs.iloc[:, self._training_indices]
        output_columns = []
        if len(self._training_indices) > 0:
            statistical_mean_abs_output = self._mean_abs(statistical_mean_abs_input,self.hyperparams["window_size"])

            if sparse.issparse(statistical_mean_abs_output):
                statistical_mean_abs_output = statistical_mean_abs_output.toarray()
            outputs = self._wrap_predictions(inputs, statistical_mean_abs_output)
        ...
        self.logger.info('Statistical MeanAbs  Primitive returned')
    ...
    def _mean_abs(self,X,window_size):
        """ statistical mean_abs of time series sequence
           Args:
            X : DataFrame
               Time series.
        Returns:
            DataFrame
            A object with mean_abs
        """
        if(window_size==-1):
            window_size = len(X)
        ...
        for column in X.columns:
            ...
            column_mean_abs = np.zeros(len(column_value))
            for iter in range(window_size-1,len(column_value)):
                sequence = column_value[iter-window_size+1:iter+1]
                column_mean_abs[iter] = np.mean(np.abs(sequence))
            column_mean_abs[:window_size-1] = column_mean_abs[window_size-1]
            transformed_X[column + "_mean_abs"] = column_mean_abs
