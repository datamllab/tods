...
class StatisticalMaximumPrimitive(transformer.TransformerPrimitiveBase[Inputs, Outputs, Hyperparams]):
    ...
    def produce(self, *, inputs: Inputs, timeout: float = None, iterations: int = None) -> base.CallResult[Outputs]:
        ...
        statistical_maximum_input = inputs
        if self.hyperparams['use_semantic_types']:
            statistical_maximum_input = inputs.iloc[:, self._training_indices]
        output_columns = []
        if len(self._training_indices) > 0:
            statistical_maximum_output = self._maximum(statistical_maximum_input,self.hyperparams["window_size"])

            if sparse.issparse(statistical_maximum_output):
                statistical_maximum_output = statistical_maximum_output.toarray()
            outputs = self._wrap_predictions(inputs, statistical_maximum_output)
    ...
    def _maximum(self,X,window_size):
        """ statistical maximum of time series sequence
           Args:
            X : DataFrame
               Time series.
        Returns:
            DataFrame
            A object with maximum
        """
        if(window_size==-1):
            window_size = len(X)
        ...
        for column in X.columns:
            ...
            column_maximum = np.zeros(len(column_value))
