...
.. dropdown:: Control Parameter
    ...
    """
    metadata = construct_primitive_metadata(module='feature_analysis', name='statistical_var', id='StatisticalVarPrimitive', primitive_family='feature_construct', hyperparams=['window_size'], description='Time Series Decompostional')
    ...
    def _produce(self, *, inputs: Inputs, timeout: float = None, iterations: int = None) -> base.CallResult[Outputs]:
        ...
        Returns:
            Container DataFrame containing var of  time series
        """
        self.logger.info('Statistical Var  Primitive called')
        ...
        statistical_var_input = inputs
        if self.hyperparams['use_semantic_types']:
            statistical_var_input = inputs.iloc[:, self._training_indices]
        output_columns = []
        if len(self._training_indices) > 0:
            statistical_var_output = self._var(statistical_var_input,self.hyperparams["window_size"])

            if sparse.issparse(statistical_var_output):
                statistical_var_output = statistical_var_output.toarray()
            outputs = self._wrap_predictions(inputs, statistical_var_output)
        ...
        self.logger.info('Statistical Var  Primitive returned')
    ...
    def _var(self,X,window_size):
        """ statistical var of time series sequence
           Args:
            X : DataFrame
               Time series.
        Returns:
        ...
        for column in X.columns:
            ...
            column_var = np.zeros(len(column_value))
            for iter in range(window_size-1,len(column_value)):
                sequence = column_value[iter-window_size+1:iter+1]
                column_var[iter] = np.var(sequence)
            column_var[:window_size-1] = column_var[window_size-1]
            transformed_X[column + "_var"] = column_var
