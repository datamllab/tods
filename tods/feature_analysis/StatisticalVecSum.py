...
.. dropdown:: Control Parameter
    ...
    metadata = construct_primitive_metadata(module='feature_analysis', name='statistical_vec_sum', id='StatisticalVecSumPrimitive', primitive_family='feature_construct', hyperparams=['window_size'], description='Time Series Decompostional')
    ...
    def _produce(self, *, inputs: Inputs, timeout: float = None, iterations: int = None) -> base.CallResult[Outputs]:
        ...
        Returns:
            Container DataFrame containing vec_sum of  time series
        """
        self.logger.info('Statistical VecSum  Primitive called')
        ...
        statistical_vec_sum_input = inputs
        if self.hyperparams['use_semantic_types']:
            statistical_vec_sum_input = inputs.iloc[:, self._training_indices]
        output_columns = []
        if len(self._training_indices) > 0:
            statistical_vec_sum_output = self._vec_sum(statistical_vec_sum_input,self.hyperparams["window_size"])

            if sparse.issparse(statistical_vec_sum_output):
                statistical_vec_sum_output = statistical_vec_sum_output.toarray()
            outputs = self._wrap_predictions(inputs, statistical_vec_sum_output)
        ...
        self.logger.info('Statistical VecSum  Primitive returned')
    ...
    def _vec_sum(self,X,window_size):
        """ statistical vec_sum of time series sequence
           Args:
            X : DataFrame
               Time series.
        Returns:
            DataFrame
            A object with vec_sum
        """
        if(window_size==-1):
            window_size = len(X)
        ...
        for column in X.columns:
            ...
            column_vec_sum = np.zeros(len(column_value))
            for iter in range(window_size-1,len(column_value)):
                sequence = column_value[iter-window_size+1:iter+1]
                column_vec_sum[iter] = np.sum(sequence)
            column_vec_sum[:window_size-1] = column_vec_sum[window_size-1]
            transformed_X[column + "_vec_sum"] = column_vec_sum
