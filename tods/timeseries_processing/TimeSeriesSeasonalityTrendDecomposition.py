...
class TimeSeriesSeasonalityTrendDecompositionPrimitive(transformer.TransformerPrimitiveBase[Inputs, Outputs, Hyperparams]):
    """
    ...
    def produce(self, *, inputs: Inputs, timeout: float = None, iterations: int = None) -> base.CallResult[Outputs]:
        ...
        """
        self.logger.info('Time Series seasonality trend decomposition Primitive called')
        ...
        seasonality_trend_decomposition_input = inputs
        if self.hyperparams['use_semantic_types']:
            seasonality_trend_decomposition_input = inputs.iloc[:, self._training_indices]
        output_columns = []
        if len(self._training_indices) > 0:
            seasonality_trend_decomposed_output = self._seasonality_trend_decomposition(seasonality_trend_decomposition_input,
                                                                                        model=self.hyperparams["model"],
                                                                                        period=self.hyperparams["period"])
            if sparse.issparse(seasonality_trend_decomposed_output):
                seasonality_trend_decomposed_output = seasonality_trend_decomposed_output.toarray()
            outputs = self._wrap_predictions(inputs, seasonality_trend_decomposed_output)
        ...
        self.logger.info('Time Series Seasonality Trend Decomposition Primitive returned')
    ...
    def _seasonality_trend_decomposition(self,X,model,period):
        """
        ...
        for column in X.columns:
            decomposed_components = sm.tsa.seasonal_decompose(X[column], model=model,
                                                              period=period,two_sided=False)
            period = self.hyperparams['period']
            if (period > 1):
                decomposed_components.trend[0:int(period / 2)] = decomposed_components.trend[int(period / 2)]
                decomposed_components.trend[
                len(decomposed_components.trend) - int(period / 2):len(decomposed_components.trend)] = \
                decomposed_components.trend[len(decomposed_components.trend) - int(period / 2) - 1]

            transformed_X[column + "_trend"] = decomposed_components.trend
            transformed_X[column + "_seasonal"] = decomposed_components.seasonal
