...
__all__= ('MovingAverageTransformerPrimitive',)
...
class MovingAverageTransformerPrimitive(UnsupervisedLearnerPrimitiveBase[Inputs, Outputs, Params, Hyperparams]):
    """
    ...
    def produce(self, *, inputs: Inputs, timeout: float = None, iterations: int = None) -> base.CallResult[Outputs]: # pragma: no cover
       ...
       try:
          ...
          if(self.hyperparams['use_columns']==()):
              columns_to_calculate_moving_average = list(set(inputs.columns)-set(['d3mIndex','timestamp','ground_truth']))
          else:
              columns_to_calculate_moving_average = self.hyperparams['use_columns']
          for column in self._training_indices:
              outputs[inputs.columns[column]+"_moving_average"] = (inputs.iloc[:, column]).rolling(3,min_periods=1,center=True).mean()
